...
public class GA {
	...
	public void genpop(){
		...
		for(int g=ind; g<(pop.length/2);g++){
	...
	public void genpop_heur(){
		int ind=pop.length/2;
		int cn=0;
		int ml=1;
		
		for(int g=ind; g<pop.length;g++){
			int ccn=(int) (cn % 11)*ml;
			Chromosome crom = new Chromosome(parameter_dom, edges,ccn);
			cn++;
			ml=-1*ml;
			if(g<pop.length){
				pop[g]=crom;
