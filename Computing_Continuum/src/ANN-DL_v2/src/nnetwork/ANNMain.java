...
public class ANNMain {
	...
	NNetwork net;
	Topology topo;
	String type, subnettype="default", function, ofunction;
	double bias;
	int inputs, outputs, hidden, nodes, hnodes, upper, lower, clustersize=0;
	...
	public ANNMain() {
		pr=new File("nnprofile");
		storedmodel="model.csv";
		readprofile();
	}
	
	public ANNMain(String subnettype) {
		this.subnettype=subnettype;
		if(subnettype.equals("hypergraph")) {
			pr=new File("nnprofile");
			storedmodel="model-hg.csv";
		}else {
			pr=new File("nnprofile");
			storedmodel="model.csv";
		}
	...
	public Double[][] ga_train(Double[] theset, int vnfsize) {
		...
		maketopology();
		makenetwork();
		net.setclustersize(clustersize);
	...
	public void setstoredmodel(String a) {
		storedmodel=a;
	}
