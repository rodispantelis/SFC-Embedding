...
public class DCController extends Thread{
	...
	public DCController(network.FTnetwork net, Double mode) {
		this.mode=mode;
		this.net=net;
	...
	public void distribute0() {
		...
		for(int r2=(sr-1);r2>(-1);r2--) {
			...
			//check demands
			if(net.getserver(t2[0]).getavailablecpu()>=vnfgraph.minnodew &&
					net.getserver(t2[1]).getavailablecpu()>=vnfgraph.minnodew) {
							addnodes(t2[0],t2[1]);
			}
		}
	}
