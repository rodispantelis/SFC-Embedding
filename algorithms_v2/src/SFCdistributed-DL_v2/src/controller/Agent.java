...
public class Agent {
	/** agent type 
	 * set "hypergraph" for single agent in multi-domain controller, "default" for other use
	 */
	String type="default";
	/** cluster nodes */
	...
	public void compute_runNN() {
		m=new nnetwork.ANNMain(type);
		m.setclustersize(nodes.size());
		...
		int tem2=tem1+maxsfcsize;
		
		Double[] tempS=new Double[tem2];
		...
		if(fitness>=0.0) {
			for(int s=0;s<output.length;s++) {
				if(result[0][s].intValue()<0.0 || result[0][s].intValue()>=nodes.size()){
					output[s]=-1;
				}else {
	...
	public void compute_trainNN() {
		...
		for(int s=0;s<output.length;s++) {
			if(result[0][s].intValue()<0.0) {
				...
				if(result[0][s].intValue()>=nodes.size()) {
					output[s]=-1;
					fitness=-1.0;
				}else {
					output[s]=nodes.get(result[0][s].intValue())[0].intValue();//node IDs
